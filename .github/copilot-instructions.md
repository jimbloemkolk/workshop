...
## Architecture
- **packages/transform/**: Business logic - pipeline transformation, critical path, job impacts, timeline layout, aggregation, type detection
- **packages/app/**: React UI - visualization, formatting, pixel positioning only

## CLI Commands

Base: `npx tsx packages/transform/src/cli.ts <command> [options]` or `npm run transform -- <command> [options]`

### Commands

1. **pipeline** - Transform single pipeline, get full analysis
   ```bash
   npx tsx packages/transform/src/cli.ts pipeline -f <file> -i <iid> --pretty
   ```
...
2. **aggregate** - Multi-pipeline job impact analysis
   ```bash
   npx tsx packages/transform/src/cli.ts aggregate -f <file> --limit 50 --pretty
   ```
...
3. **types** - Pipeline type distribution (MR, Merge Train, Main, RC, Release)
   ```bash
   npx tsx packages/transform/src/cli.ts types -f <file> --pretty
   ```
   Output: type counts, list with detected types

4. **critical-path** - Show critical path jobs for pipeline
   ```bash
   npx tsx packages/transform/src/cli.ts critical-path -f <file> -i <iid> --pretty
   ```
   Output: ordered jobs on critical path, timing, dependency types

5. **job-impacts** - Top impactful jobs for pipeline
   ```bash
   npx tsx packages/transform/src/cli.ts job-impacts -f <file> -i <iid> --top 10 --pretty
   ```
...
6. **timeline** - Timeline layout structure
   ```bash
   npx tsx packages/transform/src/cli.ts timeline -f <file> -i <iid> --pretty
   ```
   Output: pipeline hierarchy, stages (sorted), position percentages (startPercent, widthPercent, etc)

## Data Locations
- Current: `packages/app/public/data/pipelines.json`
- Pre-merge: `packages/app/public/data-pre-merge/pipelines.json`
- Old: `packages/app/public/old/pipelines.json`
...
## Files
- CLI: `packages/transform/src/cli.ts`
- Logic: `packages/transform/src/transform.ts`, `aggregation.ts`, `pipelineType.ts`
