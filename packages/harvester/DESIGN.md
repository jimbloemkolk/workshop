...
- Backend spawns ffmpeg (`avfoundation`) writing ~60s **FLAC 48kHz mono**
  segments (`segment` muxer, continued numbering across runs). Each finished
  segment is a complete valid file: a crash loses at most the current minute.
- Stop = graceful ffmpeg quit, then lossless FLAC→FLAC concat to
  `recording.flac`.
- Markers: frontend sends keydown/keyup over socket.io; backend stamps
  against its recording clock (elapsed active recording time, gaps excluded).
  Sub-**300ms** taps are stored but flagged `discarded` (auditable; threshold
  is config). Unclosed holds auto-close at stop, flagged `unclosed`.
- Backend crash mid-recording → session surfaces as interrupted; UI offers
  **Resume** (new ffmpeg run, continued segment numbers, downtime recorded as
  a flagged gap between runs) or **Finalize** (concat what exists, proceed).
...
- `doctor`: ffmpeg present, avfoundation devices listable, transcriber
  callable, Agent SDK authenticated, data + vault dirs writable, 2-second
  live mic probe producing a valid file.
...
- `.env` (gitignored, same pattern as the transcriber):
  `HARVESTER_DATA_DIR`, `HARVESTER_VAULT_DIR`, `HARVESTER_PORT`,
  `HARVESTER_MIC` (avfoundation audio device index).
