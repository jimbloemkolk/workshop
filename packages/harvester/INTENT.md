...
- Backend records the mic and stamps marker events against its own recording
  clock; the frontend only sends key-down/key-up. (Side effect, not scope:
  the marker endpoint could later take input from a phone or a button.)
...
No phone or hardware capture (the seam allows it later). No realtime
