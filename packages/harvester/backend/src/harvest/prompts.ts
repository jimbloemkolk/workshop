...
export function transcriptIntro(indexedTranscript: string, markerCount: number): string {
...
export function markerTurn(m: MarkerTurnInput): string {
  ...
  return `Marker ${m.n}/${m.total}: span ${formatTime(m.startS)}–${formatTime(m.endS)}. ${hint}
