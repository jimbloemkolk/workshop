...
export async function runE2e(baseConfig: Config, opts: { noLlm: boolean }): Promise<number> {
  ...
  try {
    ...
    check('all insights anchored (or explicitly flagged)',
      insights.every((i) => i.anchored || i.status === 'proposed'))
    ...
    for (const i of insights.filter((x) => x.anchored)) {
    ...
    if (check('insight note exists', noteFile != null)) {
